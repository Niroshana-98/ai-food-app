...
export default function OrderSuccessContent() {
  ...
  useEffect(() => {
    if (orderDetails) {
      ...
      setCurrentStep(orderDetails.status in statusMap ? statusMap[orderDetails.status as keyof typeof statusMap] : 1);
    }
  }, [orderDetails]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-50 via-emerald-50 to-teal-50 relative overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute bottom-20 right-10 w-96 h-96 bg-emerald-200/20 rounded-full blur-3xl animate-pulse"></div>
      </div>
        ...
        <div className="bg-white rounded-3xl shadow-xl p-8 mb-8 border border-green-100">
          ...
          <div className="mt-8 text-center">
            <div className={`inline-flex items-center gap-3 px-6 py-3 rounded-xl border ${getStatusColor(orderDetails.status)}`}>
              <span className="font-semibold">Current Status: {getStatusText(orderDetails.status)}</span>
            </div>
